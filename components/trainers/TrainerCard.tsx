...
const TrainerCard = ({
  ...
  return (
    <div className="w-[48%] md:w-[250px] h-[clamp(200px,56.0416668vw,330px)] sm:h-[330px] md:h-[350px] relative bg-[#edce38] rounded-2xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
      <Link
        href={`/profile/user/trainer/${agent.id}`}
        className="w-full h-full flex items-center justify-center"
      >
        {/* top section */}
        <div className="relative flex items-center justify-center w-[88%] h-[88%] bg-white mx-auto rounded-lg overflow-hidden">
          <div className="w-full h-full">
            <Image
              fill
              src={agent?.user?.image || "/images/placeholder-profile.jpg"}
              alt={agent.name}
          ...
          <div className="w-full h-[clamp(60px,12.1666666vw,80px)] md:h-[80px] relative flex items-start justify-start flex-col">
            ...
            <div className="z-30 w-full flex flex-col relative gap-[clamp(3px,0.41666667vw,8px)] md:gap-2 px-[10px]">
              <div className="h-8 flex-col justify-center items-start gap-[3px] md:gap-1.5 inline-flex">
                <div className="text-nowrap text-white text-[8px] md:text-[10.25px] font-bold font-['Montserrat'] drop-shadow-sm">
                  {agent?.user?.name}
                </div>
                <div className="w-[52px] px-0.5 pt-[2.32px] pb-[2.14px] bg-white rounded-[2.90px] justify-center items-center inline-flex overflow-hidden shadow-sm">
              ...
              <div className="flex w-full items-center justify-between">
                <div className="w-[46.81px] h-[21.42px] flex-col justify-start items-start md:gap-[4.42px] inline-flex">
                  <div className="w-[68.89px] text-white text-[6.18px] font-normal font-['Montserrat'] opacity-90">
                    {t("agent_code")}
                  </div>
        ...
        {/* abslute marks */}
        <div className="absolute flex items-center justify-center rounded-br-[5px] w-[82px] h-[25px] bg-[#239D60] left-[0px] top-[0px] text-white text-[8.30px] font-bold font-['Montserrat'] uppercase shadow-md">
          minutes 90
        </div>
        {agent?.fifa_certificate === 1 && (
          <div className="absolute right-0 top-0 w-[77px] h-[25px] bg-gradient-to-l from-[#3579b6] to-[#071a59] rounded-tr-[10px] rounded-bl-[5px] flex items-center justify-center gap-1 shadow-md">
